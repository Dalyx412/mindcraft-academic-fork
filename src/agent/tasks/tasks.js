...
export class Task {
    constructor(agent, task_data, taskStartTime = null) {
        this.agent = agent;
        // Per-profile role for multi-bot tasks (e.g. food_collector, squad_leader)
        this.task_role = agent.prompter?.profile?.task_role || null;
        this.data = null;
    ...
    getAgentGoal() {
        ...
        if (this.task_type === 'cooking') {
            ...
            if (this.data.agent_count > 2) {
                ...
                const isFoodCollector = this.task_role === 'food_collector'
                    || this.name.toLowerCase().startsWith('andy'); // legacy eval profiles
                if (isFoodCollector) {
                    ...
                    add_string = '\nIn the end, all the food items should be given to one single bot with task_role "food_collector" (or whose name starts with andy/Andy in legacy setups). Make sure to talk to all the agents using startConversation command to coordinate the task instead of talking to just one agent. You can even end current conversation with any agent using endConversation command and then talk to a new agent using startConversation command.';
                }   
            } 
            else {
