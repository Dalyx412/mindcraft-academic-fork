...
import { dispatchVoiceCommand as routeVoiceCommand } from './voice_dispatch.js';
...
export function createMindServer(host_public = false, port = 8080) {
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('voice-command', (message) => {
            const result = routeVoiceCommand(agent_connections, message);
            if (!result.ok) {
                console.warn('[VoiceCommand] No agents received message.');
            }
        });
...
export const getAgentConnections = () => agent_connections;
export { dispatchVoiceCommand } from './voice_dispatch.js';