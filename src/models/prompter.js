...
import { getReplyLanguageInstruction } from '../utils/language.js';
...
export class Prompter {
    constructor(agent, profile) {
        ...
        this._chat_model_profile = chat_model_profile;
        this._embedding_model_profile = null;
        if (this.profile.embedding) {
            try {
                this._embedding_model_profile = selectAPI(this.profile.embedding);
            } catch (e) {
                this._embedding_model_profile = null;
            }
        }
    ...
    async _resolveEmbeddingModel() {
        const candidates = [];
        if (this._embedding_model_profile) {
            candidates.push(this._embedding_model_profile);
        }
        candidates.push({ api: this._chat_model_profile.api, api_key: this._chat_model_profile.api_key });

        for (const prof of candidates) {
            const label = prof.model ? `${prof.api}/${prof.model}` : `${prof.api} (default)`;
            try {
                const model = createModel(JSON.parse(JSON.stringify(prof)));
                await model.embed('ping');
                console.log(`Embedding model ready: ${label}`);
                return model;
            } catch (err) {
                console.warn(`Embedding probe failed (${label}): ${err.message}`);
            }
        }
        try {
            await this.chat_model.embed('ping');
            console.log('Embedding model ready: chat model fallback');
            return this.chat_model;
        } catch (err) {
            console.warn(`Embedding probe failed (chat model): ${err.message}`);
        }
        console.warn('No embedding API available; using word-overlap for example selection.');
        return null;
    }

    async initExamples() {
        try {
            this.embedding_model = await this._resolveEmbeddingModel();
            this.skill_libary.embedding_model = this.embedding_model;
    ...
    async replaceStrings(prompt, messages, examples=null, to_summarize=[], last_goals=null) {
        ...
        if (prompt.includes('$SELF_PROMPT')) {
            const parts = [];
            if (this.profile.self_prompt?.trim()) {
                parts.push(this.profile.self_prompt.trim());
            }
            if (!this.agent.self_prompter.isStopped()) {
                parts.push(`YOUR CURRENT ASSIGNED GOAL: "${this.agent.self_prompter.prompt}"`);
            }
            prompt = prompt.replaceAll('$SELF_PROMPT', parts.length ? parts.join('\n') + '\n' : '');
        }
        if (prompt.includes('$REPLY_LANGUAGE')) {
            prompt = prompt.replaceAll('$REPLY_LANGUAGE', getReplyLanguageInstruction(this.profile));
        }
        if (prompt.includes('$WORLD_CONTEXT')) {
            const ctx = [
                settings.project_world_context?.trim(),
                this.profile.world_context?.trim()
            ].filter(Boolean).join('\n\n');
            prompt = prompt.replaceAll('$WORLD_CONTEXT', ctx ? ctx + '\n' : '');
        }
