...
6. Run `node main.js` (or `npm start`). Two agents (`xiaoming` / 小明, `xiaohong` / 小红) connect via Gateway using `claude-sonnet-4-6`.
...
| Agent profiles | `profiles/xiaoming_gateway.json`, `profiles/xiaohong_gateway.json` |
...
- `@xiaoming ...` — only xiaoming receives the message
- `@xiaohong ...` — only xiaohong receives the message
...
Two agents with contrasting behavior: **xiaoming / 小明** (reliable executor) and **xiaohong / 小红** (intentional “wobble”). The PM (human player) commands via `@` mentions. See `統合版_Final.md` for the full study design.

To switch to the Japanese agent pair (Aki / Haru), swap `profiles`, `init_message`, and `ui_language` in `settings.js` (see comments in that file).
